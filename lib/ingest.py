...
# from lib.constants import CHROMA_SETTINGS
...
class Ingest:
	...
	def __init__(self, profile='main', model='main', db='main', root_dir='./'):
		...
		# self.profiles.set_root(f'{root_dir}{slash}profiles')
		...
		# Create embeddings
		self.embeddings = HuggingFaceEmbeddings(model_name=self.model['embeddings_model'])

	def run(self):
		db_path = self.p.db_path(self.db_name)
		settings = Settings(**self.p.dbs[self.profile_name]['client_settings'])
		settings.persist_directory = db_path #because of langchain bug

		if self.does_vectorstore_exist(db_path):
			# Update and store locally vectorstore
			print(f"Appending to existing vectorstore at {db_path}")
			db = Chroma(persist_directory=db_path, embedding_function=self.embeddings, client_settings=settings)
			collection = db.get()
			...
			print(f"Creating embeddings. May take some minutes... {db_path}")
	...
	def process_documents(self, ignored_files: List[str] = []) -> List[Document]:
		...
		ingest_path = self.p.ingest_path(self.db_name)

		print(f"Loading documents from {ingest_path}")
